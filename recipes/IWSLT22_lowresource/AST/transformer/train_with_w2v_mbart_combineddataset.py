    ...
    def make_dataloader(
        ...
        **loader_kwargs : dict
        ...
        if "collate_fn" not in loader_kwargs:
            loader_kwargs["collate_fn"] = sb.dataio.batch.PaddedBatch

        # TRAIN stage is handled specially.
        if stage == sb.Stage.TRAIN:
            loader_kwargs = self._train_loader_specifics(dataset, loader_kwargs)
            dataloader = sb.dataio.dataloader.SaveableDataLoaderForCombinedDataset(
                dataset, **loader_kwargs
            )
        else:
            dataloader = sb.dataio.dataloader.SaveableDataLoader(
                dataset, **loader_kwargs
            )
        ...
        if (
            ...
            and (
                isinstance(
                    dataloader,
                    sb.dataio.dataloader.SaveableDataLoaderForCombinedDataset,
                )
                or isinstance(
                    dataloader, sb.dataio.dataloader.SaveableDataLoader
                )
...
def dataio_prepare(hparams, tokenizer):
    ...
    for dataset in ["train"]:
        json_paths = hparams[f"annotation_{dataset}"]
