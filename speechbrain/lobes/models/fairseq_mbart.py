"""This lobe enables the integration of fairseq pretrained mbart models.
...
class FairseqmBART(nn.Module):
    ...
    def forward(self, b_input):
        """Takes an input tensor and return the translation prediction.
        Arguments
        ---------
        b_input : torch.Tensor (input to the bart module)
            A batch of input tensors to transform to high level presentations.
        """

        # If we freeze, we simply remove all grads and features from the graph.
        if self.freeze:
            with torch.no_grad():
                return self.model(b_input).detach()

        return self.model(b_input)
